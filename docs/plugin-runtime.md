...
`codex plugin add` prints it as the installed plugin root. When the runtime is missing,
the hooks print this command to standard error and nothing to standard output, so a
host that passes only standard output to the model opens the session silently.
