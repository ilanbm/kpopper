...
this repository's record with the native `kpop` built from the same tree. The build is
cached by the hash of `native/`, so only a pull request that changes the native sources
builds it, and that pull request checks the record with the reader it changes. `check.yml` runs
