...
**The starting offer, once.** Where the workspace has no record and the work is clearly going to be revisited, put the three starting choices to the user once, briefly, with the first finding worth keeping and not before the work has begun - or, where urgent work deferred it, at the next quiet moment, even once the record exists: **learn while working** (the default), **map existing materials**, or **investigate more deeply**. Allow skipping; never on a greeting, not in the middle of urgent work, and not at all where explanations are off (`kpopper config --guidance off`, or the opener says so). No answer means learn while working, not permission to scan.
After showing it run `kpopper _agent shown welcome`, so no later session repeats it - never before or instead of showing it; `kpopper _agent status` says whether it was already offered here. A chosen map or investigation is the [map skill](../map/SKILL.md)'s work; mapping stays available on request either way.
