...
**The starting offer, once.** Where the workspace has no record and the work is clearly going to be revisited, put the three starting choices to the user once, briefly, with the first finding worth keeping and not before the work has begun: **learn while working** (the default), **map existing materials**, or **investigate more deeply**. Allow skipping; never on a greeting, not in the middle of urgent work, and not at all where explanations are off (`kpopper config --guidance off`, or the opener says so). No answer means learn while working, not permission to scan.
