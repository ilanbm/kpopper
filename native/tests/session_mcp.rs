...
fn tool() -> Tool {
    Tool::new(
        "echo",
        "Echo x",
        json!({"type":"object","properties":{"x":{"type":"integer"}},"required":["x"],"additionalProperties":false}),
    )
}
...
fn handshake_list_call_preserves_number_id() {
    ...
    server(&mut reader, &mut output, &[tool()], |_, args| {
        Ok(args["x"].to_string())
    })
    .unwrap();
...
fn invalid_args_and_application_errors_are_separate() {
    ...
    let mut reader = Cursor::new(wire);
    let mut output = Vec::new();
    server(&mut reader, &mut output, &[tool()], |_, _| {
        Err("application failed".into())
    })
    .unwrap();
    let responses: Vec<Value> = String::from_utf8(output)
        .unwrap()
