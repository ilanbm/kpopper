...
fn record(path: &Path) {
    fs::write(path.join("GROUNDING.yaml"), r#"meta:
  updated: 2026-09-01
sources:
  s.old:
...
fn run(root: &Path, state: &Path, report: &Value) -> Output {
    let mut child = Command::new(env!("CARGO_BIN_EXE_kpop-native"))
        .args(["--workspace", root.to_str().unwrap(), "update", "--file", "-", "--state-dir", state.to_str().unwrap()])
...
fn hash(path: &Path) -> String { kpop_native::identity::sha256(&fs::read(path).unwrap()) }
...
fn source_and_two_dependent_writes_publish_once_and_retry_exactly() {
    ...
    assert!(first.status.success(), "{}{}", String::from_utf8_lossy(&first.stdout), String::from_utf8_lossy(&first.stderr));
...
fn late_failure_and_private_report_retain_source_without_shared_changes() {
    for (name, extra, last) in [
        ("late", json!({}), json!({"kind":"add","id":"c.bad","body":{"rests_on":["p.missing"],"verdict":"bad","wrong_if":"p.missing > 0"}})),
        ("private", json!({"privacy":"private"}), json!({"kind":"add","id":"p.ok","body":{"v":2}})),
    ] {
        let temp = tempfile::tempdir().unwrap();
        record(temp.path());
        let entry = temp.path().join("GROUNDING.yaml");
        ...
        report.as_object_mut().unwrap().extend(extra.as_object().unwrap().clone());
        ...
        assert_eq!(fs::read(receipt["source_file"].as_str().unwrap()).unwrap(), b"retained");
    }
}
