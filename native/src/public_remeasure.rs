...
fn run_recipe(argv: &[String], root: &Path) -> Result<String> {
    let exe = if Path::new(&argv[0]).components().count() > 1 { root.join(&argv[0]) } else { PathBuf::from(&argv[0]) };
...
pub fn run(options: &Options, cwd: &Path, frozen: bool) -> Result<String> {
    ...
    for (name, ids) in &named { let argv = recipes.get(name).ok_or_else(|| Error(format!("refused - recipe {name} is not held by .kpopper/measure.yaml")))?; out.push(format!("  {} <- {}: {}", ids.join(", "), name, argv[0])); }
