...
fn state(world: &World<'_>, id: &str) -> crate::Result<(String, String)> {
    let body = &world.judgments[id];
    let fields = world.reader.fields();
    let deps = names(get(body, text(&fields["deps"])?));
    let pred = R::predicate_of(body, fields);
    let blocked = A::blocked_text(body);
    let missing = deps
        .iter()
    ...
    if !named.is_empty() && evaluated == Some(true) {
        return result(
            ...
            format!(
                ...
                P::predicate_text(&pred)
            ),
        );
    }
    let snapshot = text(&fields["snapshot"])?;
    let seen = get(body, snapshot);
    let unchecked = deps
        .iter()
    ...
    if let Some((dep, old, new, _)) = moves.iter().find(|(_, _, _, s)| *s == "moved") {
        ...
        if matches!(get(snapshot, "computed"), V::Map(_))
            && !crate::ordinary_counts::same_rule(
                get(get(snapshot, "computed"), "rule"),
                get(current, "rule"),
            )
        {
            return result("MOVED", format!("{dep}: formula changed since review"));
        }
        if crate::ordinary_counts::legacy_rule(snapshot, get(current, "rule")).is_some() {
            return result(
                "MOVED",
                format!(
        ...
        let (was, now) = P::apart(old, new, 40);
        return result(
            "MOVED",
            format!(
    ...
    if named.is_empty() && blocked.is_empty() {
        ...
        return if reopened.is_empty() {
            ...
            result(
                "HOLDS",
                format!("decided; reopened by {}", P::short(&V::Text(reopened), 80)),
            )
        };
    }
    if named.is_empty() {
        return result(
            ...
            format!(
                ...
                P::short(&V::Text(blocked), 80)
            ),
        );
    }
    let short = P::short(&pred, 80);
