...
pub(crate) fn number(v: &V) -> Option<(num_bigint::BigInt, num_bigint::BigInt)> {
    ...
    let value = V::from_typed(&crate::reasoning_scope::query_value(&v.try_typed().ok()?, 0).ok()?);
    let m = map(&value).ok()?;
    Some((
