...
pub fn write(path: &Path, fields: &Value) -> Result<()> {
    ...
    let tmp = parent.join(format!(".first-use-{}.tmp", std::process::id()));
    let mut f = OpenOptions::new().write(true).create_new(true).open(&tmp)?;
    f.write_all(serde_json::to_string_pretty(&Value::Object(out))?.as_bytes())?; f.write_all(b"\n")?; f.sync_all()?; drop(f);
    fs::rename(&tmp, path).or_else(|e| { let _ = fs::remove_file(&tmp); Err(e.into()) })
}
