...
pub(crate) fn flags(reader: &Reader<'_>, body: &V) -> Result<BTreeSet<&'static str>> {
    ...
    let rs = predicate_refs(&pred)
        ...
        .collect::<BTreeSet<_>>();
    let named = !rs.is_empty() && crate::ordinary_reader::why_undecided(&pred).is_empty();
    ...
    for (d, old) in seen {
        ...
        if reader.ids.contains(d) && (matches!(rule, V::Map(_)) || computed.is_some()) {
            ...
            let equal = crate::reasoning_authoring::same(previous, &now)?;
    ...
    for (id, claims) in holders {
        if claims[1..].iter().any(|c| {
            if matches!(c, V::Map(_) | V::List(_)) || matches!(claims[0], V::Map(_) | V::List(_)) {
                !crate::source_clock::python_equal(c, &claims[0])
            } else {
                !crate::ordinary_reader::same_legacy(c, &claims[0])
            }
        }) {
            contested.insert(id);
        }
