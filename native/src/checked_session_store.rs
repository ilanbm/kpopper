...
impl CheckedSessionStore {
    ...
    pub fn open(
        ...
        ensure_dir(&root)?;
        let root = root.canonicalize().map_err(|e| Error(format!("state root: {e}")))?;
        let input = absolute_path(input.as_ref())?;
        ...
        if let Some(value) = held {
            ...
            let held = atomic_create(&path, &ordinary(&identity)?)?;
            require(held == ordinary(&identity)?, "state directory belongs to another project/input")?;
        }
        Ok(Self {
            root,
            identity,
    ...
    pub fn save(&self, context: &CapturedAssessment, freshness: &Snapshot) -> Result<String> {
        ...
        let session = CheckedSession::new(context.clone(), session_identity.clone(), self.profile.as_ref())?;
        require(
    ...
    pub fn load(&self, revision: &str, freshness: &Snapshot) -> Result<CheckedSession> {
        ...
        CheckedSession::new(context, session_identity, self.profile.as_ref())
    }

    pub fn context_path(&self, revision: &str) -> Result<PathBuf> {
        require(valid_revision(revision), "invalid core session revision")?;
        Ok(self.root.join(format!("core-context-{revision}.json")))
    }
    fn session_identity(&self) -> Result<V> {
        let project = match &self.identity {
            V::Map(m) => m.get("project").cloned(),
            _ => None,
        }.ok_or_else(|| Error("invalid project identity".into()))?;
        let mut m = std::collections::BTreeMap::from([("project".into(), project)]);
        m.insert("navigation_profile_sha256".into(), self.profile.as_ref().map(profile_digest).transpose()?.map_or(V::Null, V::Text));
        Ok(V::Map(m))
    }
    fn validate_root(&self) -> Result<()> { ensure_dir(&self.root)?; require(self.root.canonicalize().map_err(|e| Error(e.to_string()))? == self.root, "state root changed") }
}
...
fn absolute_path(path: &Path) -> Result<PathBuf> {
    if path.is_absolute() { Ok(path.to_path_buf()) } else { Ok(std::env::current_dir().map_err(|e| Error(e.to_string()))?.join(path)) }
}
fn profile_digest(v: &V) -> Result<String> {
    Ok(sha256(serde_json::to_vec(&ordinary(v)?)?.as_slice()))
}
fn ensure_dir(path: &Path) -> Result<()> {
    require(path.is_absolute(), "state root must be absolute")?;
    ...
    #[cfg(unix)] { use std::os::unix::fs::PermissionsExt; fs::set_permissions(path, fs::Permissions::from_mode(0o700)).map_err(|e| Error(e.to_string()))?; }
    Ok(())
}
...
fn read_json(path: &Path) -> Result<J> {
    ...
    let file = File::open(path).map_err(|e| Error(format!("retained context: {e}")))?;
    let mut bytes = Vec::with_capacity(meta.len() as usize);
    file.take(MAX_FILE + 1).read_to_end(&mut bytes)
        .map_err(|e| Error(e.to_string()))?;
    require(bytes.len() as u64 <= MAX_FILE, "retained context exceeds limit")?;
...
fn atomic_create(path: &Path, value: &J) -> Result<J> {
    ...
    let mut file = tempfile::NamedTempFile::new_in(parent).map_err(|e| Error(format!("temporary state: {e}")))?;
    file.write_all(&bytes)
        .and_then(|_| file.as_file().sync_all())
        .map_err(|e| Error(format!("temporary state: {e}")))?;
    #[cfg(unix)] { use std::os::unix::fs::PermissionsExt; file.as_file().set_permissions(fs::Permissions::from_mode(0o600)).map_err(|e| Error(e.to_string()))?; }
