//! Source-order record loading under complete participant read guards.
use crate::{
    Result,
    history_contract::*,
...
pub(crate) fn load(paths: &[PathBuf], inventory: &mut Inventory) -> Result<Document> {
        ...
        document.history_view = Some(V::Map(Map::from([
            ...
            (
                ...
                V::Text(crate::reasoning_snapshot::digest(
                    &map(&map(&capture.document)?["meta"])?["history"],
                )?),
            ),
        ])));
    ...
    if let Some(capture) = &document.history {
            ...
            .filter(|v| **v != V::Null)
        {
            ...
            for item in crate::history_view::list(&map(imported)?["physical"])? {
                let item = map(item)?;
        ...
        let (named, _) =
            crate::history_hypotheses::layers(document.history_projection.as_ref().unwrap(), &doc)?;
