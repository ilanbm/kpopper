...
environment Copilot starts hooks with, `hook.sh` runs `hook.py` with `python3`, which
then needs kpopper's dependencies. See
[Python compatibility mode](../../docs/plugin-runtime.md#python-compatibility-mode).
Configurations generated by `python3 hook.py config` keep running `hook.py` directly;
regenerate them with `hook.sh` to use the native runtime.
...
context, and selects Python only when named.

`python3 -m unittest discover -s tests -p test_copilot_adapter.py` checks the Python
bridge, `hook.py`: payload workspace selection, opening context and session identity,
silent legacy stops, resume preserving the original baseline, first use without
creating a record, and its configuration generation.

The native bridge has not yet run inside Copilot CLI; the runs below used the Python
bridge. A separate offline CLI run used isolated `COPILOT_HOME` and `COPILOT_CACHE_HOME`, a
