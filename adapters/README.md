...
|---|---|---|---|---|
| Claude Code *(baseline, unchanged)* | `SessionStart` hook, blocking-capable | `Stop` hook, exit 2 blocks, bounces once | `skills/*/` - one skill per occasion - loaded natively by the plugin engine | `kpopper page` writes `record.html`; open in any browser |
| **Codex CLI** | `SessionStart` hook — reuses `session_open.sh` as-is | `Stop` hook — reuses `session_gate.sh` as-is, exit 2 blocks | `AGENTS.md` snippet + every `skills/*/` directory symlinked into `.agents/skills` | same `record.html`, opened by hand |
