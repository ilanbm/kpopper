...
(async () => {
  ...
  for (const theme of ['light', 'dark']) {
    const ctx = await b.newContext({ viewport: { width: 1100, height: 900 }, colorScheme: theme });
    ...
    // whatever tab you land on, hover the first value it shows
    const el = p.locator('section:not([hidden]) td [data-id]').first();
    await el.scrollIntoViewIfNeeded();
    await p.mouse.move(2, 2);
    await el.hover();
    chk(`${T} hover opens the source`, await seen(p, '.pop'));
    chk(`${T} it names where the value came from`, /from|rule|value/i.test(await txt(p)));
