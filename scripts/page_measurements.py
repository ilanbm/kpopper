...
def read(expected):
    ...
    try:
        ...
        if payload['inputs'] != expected['identity']:
            return {}, 'page measurement is stale; run kpop experimental hub --verify again'
        return _counts(payload['counts']), 'the selected page did not produce this count'
    except FileNotFoundError:
        return {}, 'no measurement for this record and view; run kpop experimental hub --verify'
