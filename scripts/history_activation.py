...
def _guard(entry, inventory, expected_digests, deployment_guard, *, members=()):
...
def _trees(entry):
    layout = P.layout(entry)
    return [Path(layout[role]).relative_to(entry.parent).as_posix() for role in ('history', 'history_commits')] + [M.ARTIFACTS]
...
def prepare_activation(entry, *, inventory, expected_digests, deployment_guard,
                       operation=None, recorded_at=None, record_id=None):
    ...
    with _guard(entry, inventory, expected_digests, deployment_guard, members=members) as project:
        ...
        C._require(not _tree(entry.parent, _trees(entry)[1]), 'history_reactivation_requires_generation_mapping')
        C._require(not (entry.parent / M.ARTIFACTS).exists(), 'existing_activation_evidence')
...
def _read_witnesses(entry, mutation):
    ...
    for relative, expected in baseline['source_files'].items():
        ...
        if item is not None:
            C._require(raw in (item['before'], item['after']), 'transition_source_changed', relative)
        else:
...
def _candidate_isolated(entry, mutation):
    ...
    if baseline['direction'] == 'activate':
        ...
        original_bytes = next(item['after'] for item in files if item['path'] == M.ARTIFACTS + '/original.json')
        ...
        with tempfile.TemporaryDirectory(prefix='history-transition-verify-') as temporary:
            ...
            for relative, expected in baseline['source_files'].items():
                ...
                C._require(raw is not None and C.sha256(raw) == expected, 'transition_source_changed', relative)
                retained = touched.get(M.ARTIFACTS + '/originals/' + relative)
                C._require(retained is not None and retained['after'] == raw, 'transition_original_mismatch', relative)
            ...
            for member in expected_template['meta']['history_import']['members']:
                if member['path'] == M.ARTIFACTS + '/original.json':
            ...
            if observation is not None:
                live_item = touched.get(M.ARTIFACTS + '/live.json')
                ...
                candidate_item = touched.get(M.ARTIFACTS + '/candidate.json')
                ...
                expected_template['meta']['history_import']['members'].append({
                    'path': M.ARTIFACTS + '/live.json', 'sha256': observation['sha256'], 'role': 'retained_original'})
...
def _verify(entry, mutation, project):
    data = mutation.to_data()
...
def prepare_deactivation(entry, activation, *, inventory, expected_digests, deployment_guard,
    ...
    with _guard(entry, inventory, expected_digests, deployment_guard, members=members) as project:
        ...
        original_snapshot = next(item['after'] for item in activation.files
                                 if item['path'] == M.ARTIFACTS + '/original.json')
        marker = C.authority(record_id=captured.marker['record_id'], authority='legacy',
                             generation=captured.marker['generation'] + 1)
...
def _same_live_context(actual, expected):
    ...
    C._require(G.identity(left['hypotheses']) == G.identity(right['hypotheses']),
...
def publish(entry, mutation, *, deployment_guard):
    ...
    with _guard(entry, deployment['inventory'], deployment['expected_digests'], deployment_guard,
        ...
        T.publish_transition(entry.parent, T.journal_for(entry), mutation,
                             ...
                             on_committed=lambda unused: _verify(entry, mutation, project))
