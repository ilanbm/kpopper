## What changed

<!-- A sentence or two, as a change to the product. -->

## Why

## How it was exercised

<!-- The commands, or the path through the page. Every pull request already runs
     `kpopper check` and `kpopper page --verify`; this is for what those cannot see. -->

Bump: minor

<!-- patch: a fix, a doc, tooling - nothing a user of the reader, the page or the hooks
     has to learn. minor: something added - a command, a field the reader accepts, a
     computed name, a page behaviour. major: something removed or changed in meaning.
     The release pull request takes the largest bump merged since the last release. -->

---

- [ ] A decision this change makes is written into `GROUNDING.yaml`, with what would make it wrong — a sign the change itself cannot suppress — or it makes none
