...
class SessionActivity(unittest.TestCase):
    ...
    def hook(self, **payload):
        return subprocess.run(['sh', str(ROOT / 'scripts/session_gate.sh'), '--host', 'codex'],
            input=json.dumps({'session_id': self.sid, 'cwd': str(self.work), **payload}),
    ...
    def test_imported_commit_is_not_session_authorship(self):
        ...
        self.assertNotIn('this session wrote', stopped.stderr)
    ...
    def test_owned_write_still_warns_after_commit_and_delivers_once(self):
        ...
        self.assertIn('p.new', first.stderr)
    ...
    def test_new_problem_warns_without_repeating_old_problem(self):
        ...
        self.assertIn('p.second', again.stderr)
        self.assertNotIn('p.first', again.stderr)
    ...
    def test_named_hypothesis_write_is_owned(self):
        ...
        first = self.hook()
        self.assertEqual(first.returncode, 2, first.stderr)
        self.assertIn('p.hypothesis', first.stderr)
        self.assertEqual(self.hook().returncode, 0)
    ...
    def test_concurrent_stops_deliver_each_problem_once(self):
        self.add()
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            results = list(pool.map(lambda _: self.hook(), range(2)))
        self.assertEqual(sorted(p.returncode for p in results), [0, 2], [p.stderr for p in results])

    def test_cli_session_binding_and_host_recursion_flag(self):
        ...
        self.assertEqual(self.hook(stop_hook_active=True).returncode, 2)
        self.assertEqual(self.hook().returncode, 0)
    ...
    def test_active_history_direct_write_is_owned(self):
        ...
        self.assertEqual(first.returncode, 2, first.stderr)
        self.assertIn('p.new', first.stderr)
        self.assertNotIn('p.imported', first.stderr)
        self.assertEqual(self.hook().returncode, 0)
    ...
    def test_core_capture_failure_is_delivered_once_and_remains_invalid(self):
        ...
        first = self.hook()
        self.assertEqual(first.returncode, 2, first.stderr)
        self.assertIn('capture failure', first.stderr)
        self.assertEqual(self.hook().returncode, 0)
        self.assertEqual(self.gate()[0], 2)
