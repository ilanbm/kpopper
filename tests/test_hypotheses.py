...
class TheWritePathForks(unittest.TestCase):
    ...
    def test_review_in_a_hypothesis_writes_inside_the_braces_of_a_judgment_written_on_one_line(self):
        # nothing is written under it, where it would be read as another judgment of the
        # hypothesis (four spaces) or break its file (two)
        judgment = 'verdict: known, requires: [api.limit], fails_if: "api.limit > 100"'
        for pad in ("  ", "    "):
            for body, want in ((judgment, judgment + ", seen: {api.limit: 10}"),
                ...
                with tempfile.TemporaryDirectory() as d:
                    ...
                    head = f'hypothesis: {{born: "2025-12-01"}}\n\njudgments:\n{pad}d.w: {{'
                    h.write_text(head + body + "}\n", encoding="utf-8")
                    ...
                    self.assertEqual(h.read_text(encoding="utf-8"), head + want + "}\n")
