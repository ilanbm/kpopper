...
from scripts.reasoning.runtime import target_name
...
for name in ('single', 'act', 'proposal', 'batch'):
...
class RetainedSourceOracle(unittest.TestCase):
    ...
    def test_exact_pinned_source_v2_v3_replay_retains_original_bytes(self):
        for version in (2, 3):
            with self.subTest(version=version):
                ...
                if target_name() != self.target:
                    ...
                    continue
                A.commit(entry, mutation, verify=lambda data: None)
                A.commit(entry, T.PreparedMutation.from_bytes(raw), verify=lambda data: None)
                self.assertEqual(mutation.to_bytes(), raw)
                manifest = next(item for item in mutation.files if item['role'] == 'history_commit')
                self.assertEqual((entry.parent / manifest['path']).read_bytes(), manifest['after'])
